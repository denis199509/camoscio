...
// Una sola linea (o un gruppo di tratti) per volta, ridisegnabile.
...
window.disegnaPercorsoSalvato = function (punti, opzioni) {
    ...
    const tratti = opzioni && Array.isArray(opzioni.tratti) ? opzioni.tratti : null;

    if (tratti) {
        // Revisione 43a (ALTO): un RouteDraft ricalcolato con /api/routing/plan puo' avere
        // tratti 'retta' (nessun sentiero conosciuto li collega, vedi lib/trailGraph.js) -
        // disegnarli identici al sentiero vero sarebbe promettere un percorso affidabile
        // dove non lo e' (vincolo hard 7). Stessa distinzione visiva GIA' in uso nel route
        // planner (COLORE_SENTIERO continuo / COLORE_RETTA tratteggiato, routeplanner.js
        // disegnaPercorso) - un utente che ha usato quello la riconosce subito qui.
        const linee = tratti.map(t => {
            const seg = t.coordinate
                .map(p => [p[1], p[0]])
                .filter(c => Number.isFinite(c[0]) && Number.isFinite(c[1]));
            if (seg.length < 2) return null;
            return t.tipo === 'sentiero'
                ? L.polyline(seg, { color: window.CAMOSCIO_COLORI.blu, weight: 4, opacity: 0.9 })
                : L.polyline(seg, { color: window.CAMOSCIO_COLORI.rosso, weight: 4, opacity: 0.95, dashArray: '8, 8' });
        }).filter(Boolean);
        if (!linee.length) return;
        savedRoutePolyline = L.featureGroup(linee).addTo(window.mapInstance);
    } else {
        ...
        // un'escursione pianificata. Un SavedRoute e' gia' una linea sola: non ha tratti
        // 'sentiero'/'retta' distinti da mostrare.
