...
async function loadTrailIndex() {
    // routingOnly: { $ne: true } TIENE FUORI I DATI DEL PUNTO 13.
    //
    // Dal 2026-07-27 sulla collezione trails non ci sono piu' solo i sentieri veri: ci
    // sono anche le carrarecce e le stradine entro 800 m dalla rete conosciuta (200 m
    // fino al 2026-08-21, punto 98/E), servite alla progettazione di un percorso
    // (livello i). Sono piu' del doppio dei dati.
    // Caricarli QUI vorrebbe dire rifare l'incidente della Fase G, che ha gia' fatto
    // cadere il sito per memoria esaurita: su Render ci sono 512 MB in tutto e il picco
    // misurato allora era gia' ~394 MB.
    // Questo indice serve ad AGGANCIARE IL GPS di chi cammina, e il GPS va agganciato ai
    // sentieri veri - non a una strada di servizio che passa li' vicino. Quindi il filtro
    // non e' solo una difesa sulla memoria: e' anche la cosa giusta nel merito.
    // La forma { $ne: true } e non { $exists: false } perche' prende correttamente sia i
    // documenti nuovi sia i 24.675 gia' presenti, che il campo non ce l'hanno affatto.
    const cursor = Trail.find(
        { region: { $in: ACTIVE_REGIONS }, routingOnly: { $ne: true } },
        { wayId: 1, coordinates: 1, _id: 0 }
    ).lean().cursor();
    ...
    for await (const trail of cursor) {
        ...
        // Ogni 500 sentieri si cede il controllo all'event loop per un istante: misurato
