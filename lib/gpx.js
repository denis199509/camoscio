...
//
// CRITICO misurato (verifica generale, blocco 2, 44a sessione) e corretto qui (45a):
// il gruppo degli attributi era [^>]*? - "qualunque carattere tranne >", quindi poteva
// attraversare un < non richiuso. Su un file malformato/costruito apposta con molte
// occorrenze di "<trkpt" senza un > vero vicino, ogni tentativo di match scorreva in
// avanti fino al PROSSIMO > reale nel file, anche a decine di migliaia di caratteri di
// distanza - ripetuto per ogni occorrenza, il costo totale diventava quadratico: 80.000
// occorrenze/0,53 MB = 25,5 secondi misurati, un file di pochi MB puo' bloccare l'event
// loop per ore (Render e' un solo processo -> tutto il sito giu', Dead Man's Switch
// compreso). [^<>]*? si ferma appena incontra il PROSSIMO < invece di attraversarlo:
// un file valido non ha mai un < non escapato dentro gli attributi (sarebbe XML non
// valido), quindi l'esito su un file valido non cambia - cambia solo la velocita' con
// cui un file rotto viene scartato. Vedi 07-Trappole-Tecniche.md nel vault.
function regexPunti(tag) {
    return new RegExp(`<(?:\\w+:)?${tag}\\b([^<>]*?)(?:/>|>([\\s\\S]*?)</(?:\\w+:)?${tag}>)`, 'gi');
}

// Legge i match uno alla volta dall'iteratore di matchAll (che e' lazy per spec) invece
// di materializzarli tutti in un colpo con [...matchAll(...)]: un file con piu' di
// MAX_PUNTI occorrenze viene scartato APPENA il tetto e' superato, senza continuare a
// far lavorare la regex sul resto del file e senza costruire comunque l'array intero
// prima di controllarne la lunghezza (quello che succedeva prima - vedi CRITICO in
// regexPunti sopra: da solo il tetto non bastava perche' il conteggio avveniva DOPO il
// lavoro costoso, non prima).
function leggiMatchEntroTetto(testo, tag) {
    const risultati = [];
    for (const m of testo.matchAll(regexPunti(tag))) {
        risultati.push(m);
        if (risultati.length > MAX_PUNTI) {
            throw new ErroreGpx(`Il file contiene piu' di ${MAX_PUNTI.toLocaleString('it-IT')} punti <${tag}>, troppi per essere una registrazione reale.`);
        }
    }
    return risultati;
}
...
function elemento(contenuto, nome) {
    ...
    if (!m) return null;
    let valore = m[1].trim();
    // Alcuni programmi avvolgono il testo libero (tipicamente <name>) in CDATA anche
    // senza bisogno - trovato dal vivo (punto 102): un file .gpx di DaniWoll aveva il
    // titolo salvato come "<![CDATA[Pizzo Cefalone...]]>", mostrato cosi' com'era invece
    // del solo testo. Non e' risoluzione di entita' (che qui resta vietata, vedi
    // l'intestazione del file): CDATA dice solo "non interpretare questo testo come
    // markup", toglierne i delimitatori non fa analizzare nulla in piu'.
    const cdata = /^<!\[CDATA\[([\s\S]*)\]\]>$/.exec(valore);
    if (cdata) valore = cdata[1].trim();
    return valore;
}
...
function parseGpx(testo) {
    ...
    let grezzi = leggiMatchEntroTetto(testo, 'trkpt');
    let tipo = 'traccia';
    if (grezzi.length === 0) {
        grezzi = leggiMatchEntroTetto(testo, 'rtept');
        tipo = 'itinerario';
    }
    if (grezzi.length === 0) {
    ...
    return {
        punti,
        nome: nomeDelPercorso(testo, grezzi[0] ? grezzi[0].index : testo.length, tipo),
...
// Il titolo dell'uscita importata. Prima era il PRIMO <name> dell'intero file, ma molti
// programmi scrivono i <wpt> prima del <trk>: il titolo diventava il nome di un waypoint
// privato ("Casa", "Parcheggio") e finiva nel feed dei follower (verifica generale, 57a
// sessione). Ora, solo nel testo prima del primo punto: il <name> dell'intestazione del
// <trk>/<rte> che contiene quel punto, altrimenti quello di <metadata>, MAI quello di un
// <wpt>. Nessun titolo -> null, e il chiamante usa la data.
// Solo ricerche lineari (search, un giro di matchAll con [^<>]*): questo file ha gia'
// avuto un CRITICO da backtracking, vedi regexPunti sopra.
function nomeDelPercorso(testo, indicePrimoPunto, tipo) {
    const intestazione = testo.slice(0, indicePrimoPunto);
    const contenitore = tipo === 'itinerario' ? 'rte' : 'trk';
    let ultimaApertura = null;
    for (const m of intestazione.matchAll(new RegExp(`<(?:\\w+:)?${contenitore}\\b[^<>]*>`, 'gi'))) {
        ultimaApertura = m;
    }
    if (ultimaApertura) {
        const nome = elemento(intestazione.slice(ultimaApertura.index + ultimaApertura[0].length), 'name');
        if (nome) return nome;
    }
    const inizioMeta = intestazione.search(/<(?:\w+:)?metadata\b/i);
    if (inizioMeta < 0) return null;
    const dopoMeta = intestazione.slice(inizioMeta);
    const fineMeta = dopoMeta.search(/<\/(?:\w+:)?metadata>/i);
    if (fineMeta < 0) return null;
    // Via <author>: il suo <name> e' il nome di una PERSONA, non del percorso, e dentro
    // <metadata> puo' comparire quando il file non ha un <name> suo.
    let meta = dopoMeta.slice(0, fineMeta);
    const inizioAutore = meta.search(/<(?:\w+:)?author\b/i);
    if (inizioAutore >= 0) {
        const fineAutore = meta.search(/<\/(?:\w+:)?author>/i);
        meta = meta.slice(0, inizioAutore) + (fineAutore > inizioAutore ? meta.slice(fineAutore) : '');
    }
    return elemento(meta, 'name') || null;
}
...
// perche' la salita venga contata. STAVA IN routes/tracking.js, spostata qui col punto 33.
//
// DAL 2026-09-06 CI SONO DUE VALORI, per decisione esplicita di Denis ("mi fido piu' della
// registrazione Garmin", "se importo una traccia voglio vedere i dati della traccia senza
// correzioni"):
//   - SOGLIA_DISLIVELLO_M = 10 -> per le quote STIMATE: percorsi progettati (lib/elevation.js,
//     quote interpolate da un modello a celle di 90 m, il rumore e' vero e va tolto) e
//     tracciamento GPS dal vivo (telefono senza barometro, sbaglia di parecchi metri fra un
//     fix e l'altro).
//   - SOGLIA_DISLIVELLO_IMPORT_M = 0 -> per le tracce IMPORTATE da un file di un dispositivo
//     (.gpx/.fit, altimetro barometrico): nessuna correzione, il dislivello e' la somma
//     grezza delle salite del file. Misurato sul .fit di Denis (Castel San Pietro): soglia
//     10 dava 1304 m, soglia 0 da' 1555 m, il Garmin ne mostra ~1503 - i 199 m erano la
//     soglia, non il campionamento.
//
// SI', COSI' LO STESSO PERCORSO PROGETTATO E POI CAMMINATO DA' DUE NUMERI. E' la scelta:
// il progetto e' una stima, la traccia una misura, e la misura vince. La FUNZIONE resta una
// sola (lezione del punto 18): a divergere e' solo il parametro, scelto dal chiamante.
//
// L'algoritmo (accumulo-da-minimo-locale, sotto) regge la soglia 0 senza il vecchio
// problema del "dislivello zero": una salita barometrica liscia accumula comunque tutto il
// suo dislivello sopra il minimo locale e viene contata - con la soglia 0 in piu' contano
// anche le micro-ondulazioni, che e' esattamente cio' che Denis vuole vedere.
//
...
// Tracce importate da un file di un dispositivo: nessuna lisciatura (vedi il blocco sopra).
const SOGLIA_DISLIVELLO_IMPORT_M = 0;
...
// e' quasi sempre un difetto rimasto aperto in una sola"). La funzione resta una: e' la
// SOGLIA passata dal chiamante a cambiare (10 per le quote stimate, 0 per le tracce
// importate da dispositivo - vedi il blocco di commento sopra SOGLIA_DISLIVELLO_M).
function statisticheTraccia(punti, sogliaDislivelloM, haversineKm) {
    ...
    return {
        ...
        // reduce, non Math.min/max(...quote): lo spread di un array oltre ~1e5 elementi lancia
        // RangeError, e una traccia multi-day registrata a 1 Hz ci arriva. Nessuno spread, nessun tetto.
        quotaMinM: quote.length ? Math.round(quote.reduce((m, q) => (q < m ? q : m), Infinity)) : null,
        quotaMaxM: quote.length ? Math.round(quote.reduce((m, q) => (q > m ? q : m), -Infinity)) : null
    };
}
...
// Avvolge tempiTraccia in modo che chiamarla non possa mai far fallire l'operazione che la
// ospita (importare un file, chiudere un tracciamento, completare un gruppo) - il tempo di
// cammino e' un di piu', non deve poter impedire quello che conta di piu'. Punto 92: esisteva
// gia' identica due volte (routes/completions.js e routes/hikes.js) prima di aggiungere qui i
// due punti nuovi di routes/tracking.js - lezione del punto 18, non farne una quarta copia.
// @returns {{ sec: number|null, motivi: string[] }}   sec = null se non ci si puo' fidare
function movimentoSecAttendibile(punti, haversineKm) {
    try {
        const tempi = tempiTraccia(punti, haversineKm);
        if (tempi.attendibile && tempi.movimentoSec > 0) {
            return { sec: tempi.movimentoSec, motivi: [] };
        }
        return { sec: null, motivi: tempi.motivi || [] };
    } catch (e) {
        console.error('Errore nel calcolo cammino/pause dal gpx:', e.message);
        return { sec: null, motivi: [] };
    }
}

module.exports = { parseGpx, statisticheTraccia, tempiTraccia, movimentoSecAttendibile, ErroreGpx, MAX_PUNTI, SOGLIA_DISLIVELLO_M, SOGLIA_DISLIVELLO_IMPORT_M };
