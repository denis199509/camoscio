...
const { parseGpx, statisticheTraccia, ErroreGpx, SOGLIA_DISLIVELLO_IMPORT_M } = require('./gpx');
...
// Punto 116 / Punto 1 - la polyline da disegnare sulla mappa, ricavata da una lista di punti
// [lng, lat, ...]. Douglas-Peucker grezzo (18 m, una linea DA VEDERE non un dato topografico)
// + riduzione a [lng, lat] + tetto a MAX_PUNTI_PERCORSO campionando a passo fisso (l'ultimo
// punto si tiene sempre). Sotto i 2 punti (una retta collassata, o una traccia degenere):
// undefined, niente linea. Usata dal ramo gpx/fit qui sotto e da misureDaSessione (la
// registrazione dal vivo del completamento di gruppo da tracciamento, routes/hikes.js).
function routePathDaPunti(punti) {
    if (!Array.isArray(punti) || punti.length < 2) return undefined;
    let path = simplifyTrack(punti, SEMPLIFICA_PERCORSO_M).map(p => [p[0], p[1]]);
    if (path.length > MAX_PUNTI_PERCORSO) {
        const q = Math.ceil(path.length / MAX_PUNTI_PERCORSO);
        path = path.filter((_, i) => i % q === 0 || i === path.length - 1);
    }
    return path.length >= 2 ? path : undefined;
}

// Punto 1 (35a) - traduce una ActiveHikeSession CONCLUSA nella stessa forma che
// calcolaDaPercorso restituisce per un file: i numeri CONDIVISI dell'escursione + i tempi +
// la linea. NON tocca il database e non solleva mai - chi chiama (routes/hikes.js
// /:id/complete-group) decide cosa farne. "sessione" dev'essere un documento Mongoose, non
// un oggetto .lean(): il virtual durationSeconds (models/ActiveHikeSession.js, gia' al netto
// di pausedMs = decisione di Denis "tempo totale al netto delle pause") non esiste sui lean.
//
// "sostanziale" = c'e' davvero una camminata dietro, non un tracciamento partito per
// sbaglio (distanza sopra 50 m, almeno 2 punti, almeno una quota). Sotto quella soglia i
// numeri della Hike e i tempi NON si toccano: si aggiorna solo "chi c'era", esattamente
// come un complete-group "a mano" senza file. La soglia 0.05 km e' la stessa gia' usata da
// public/js/storico.js per nascondere gli avvii annullati.
function misureDaSessione(sessione) {
    const punti = Array.isArray(sessione.points) ? sessione.points : [];
    const quote = punti.map(p => p[2]).filter(q => Number.isFinite(q));
    const sostanziale = (sessione.distanceKm || 0) > 0.05 && punti.length >= 2 && quote.length > 0;
    // Una registrazione dal vivo SENZA dato di elevazione: il client manda quota null
    // (tracking.js, coords.altitude assente) e sanitizePoints la sostituisce con 0. Cosi'
    // quote.length > 0 e' vero (zeri veri) ma non c'e' nessuna quota reale: maxAltitude 0 ed
    // elevationGain 0 SOVRASCRIVEREBBERO i valori messi a mano dall'organizzatore, e con
    // dislivello 0 salterebbe anche il tempo CAI della card. Il ramo file ha gia' questa
    // guardia (statisticheTraccia -> throw se quotaMaxM e' null); qui la aggiungiamo.
    const quoteVere = quote.some(q => q !== 0);

    const ore = (sessione.durationSeconds || 0) / 3600;
    // Quota massima: il campo dedicato scritto a /:id/end sui punti COMPLETI se c'e',
    // altrimenti il massimo dei punti gia' semplificati - le sessioni chiuse prima del
    // campo (35a) non ce l'hanno e non e' recuperabile con precisione.
    // reduce e non Math.max(...quote): lo spread di un array lungo (multi-day) lancia RangeError.
    const maxDaiPunti = quote.length ? Math.round(quote.reduce((m, q) => (q > m ? q : m), -Infinity)) : null;

    return {
        sostanziale,
        // maxAltitude/elevationGain: null quando non c'e' una quota reale (vedi quoteVere) -
        // il chiamante NON deve toccare i valori della Hike in quel caso.
        maxAltitude: (sostanziale && quoteVere)
            ? (Number.isFinite(sessione.maxAltitudeM) ? Math.round(sessione.maxAltitudeM) : maxDaiPunti)
            : null,
        // Dislivello dal vivo: soglia 10 m (SOGLIA_DISLIVELLO_M, gia' applicata a
        // /:id/end in elevationGainM) - NON la soglia 0 dei file di dispositivo. Sono le due
        // soglie del 06/09: 0 per un altimetro barometrico, 10 per le quote stimate dal GPS
        // di un telefono, e una registrazione dal vivo e' il secondo caso.
        elevationGain: (sostanziale && quoteVere) ? Math.round(sessione.elevationGainM || 0) : null,
        // distanceKm arrotondato: ActiveHikeSession.distanceKm e' una somma di $inc di float
        // (tracking.js) e arriva quasi sempre con la coda binaria (11.275999999999991). Ogni
        // altro scrittore arrotonda a 3 decimali; senza, la card mostrava la coda.
        distanceKm: sostanziale ? Math.round((sessione.distanceKm || 0) * 1000) / 1000 : null,
        routePath: sostanziale ? (routePathDaPunti(punti) || null) : null,
        actualTimeHours: (sostanziale && ore > 0) ? ore : null,
        movingTimeHours: (sostanziale && sessione.movingTimeSec > 0) ? (sessione.movingTimeSec / 3600) : null
    };
}
...
// B-2 (follow-up revisione sicurezza, 31a): ogni messaggio qui sotto e' scritto a mano, mai
// un valore grezzo di un utente/servizio esterno - ma chi chiama questa funzione
// (routes/hikes.js, routes/completions.js, risolviPercorso qui sotto) non aveva modo di
// saperlo: un throw new Error semplice e' indistinguibile da un errore imprevisto che
// potesse incorporare un dato non sicuro (stesso rischio gia' chiuso sui ValidationError in
// routes/auth.js). erroreUtente marca il messaggio come sicuro da mostrare cosi' com'e' -
// stesso flag .utente gia' usato da ErroreGpx/ErroreFit (lib/gpx.js, lib/fit.js, gia'
// controllato da routes/tracking.js); qui prima si perdeva ad ogni rilancio, perche' i due
// rami piu' sotto prendevano SOLO il messaggio da un ErroreGpx/ErroreFit gia' sicuro e lo
// re-incapsulavano in un Error nuovo, senza portare il flag con se'.
...
async function calcolaDaPercorso(routeSource, userId) {
    if (!routeSource || typeof routeSource !== 'object') {
        throw erroreUtente('Percorso non valido.');
    }
    ...
    if (routeSource.kind === 'draft') {
        ...
        if (!bozza) throw erroreUtente('Questo progetto non esiste o non è tuo.');
    ...
    if (routeSource.kind === 'saved') {
        ...
        if (!percorso) throw erroreUtente('Questo percorso non esiste o non è tuo.');
    ...
    if (routeSource.kind === 'gpx' || routeSource.kind === 'fit') {
        ...
        if (routeSource.kind === 'fit') {
            ...
            if (typeof b64 !== 'string' || !b64.trim()) {
                throw erroreUtente('Nessun file .fit ricevuto.');
            }
            let buf = null;
            ...
            if (buf.length > MAX_BYTE_GPX_HIKE) {
                throw erroreUtente('Il file supera i 10 MB. Una traccia normalmente pesa molto meno.');
            }
            try {
                letto = await parseFit(buf);
            } catch (e) {
                // BASSO (giro agente sul fix B-2): il ramo sicuro rimanda solo e.message, mai
                // l'errore originale - senza questa riga un fallimento INATTESO di parseFit
                // (non un file rotto, un bug vero) sparirebbe senza lasciare traccia da
                // nessuna parte (il call site logga solo quando .utente manca, e qui c'e' sempre).
                if (!(e instanceof ErroreFit) && !e.utente) console.error('parseFit inatteso:', e);
                throw erroreUtente((e instanceof ErroreFit || e.utente) ? e.message : 'File .fit non leggibile.');
            }
        } else {
            const testo = routeSource.gpxText;
            if (typeof testo !== 'string' || !testo.trim()) {
                throw erroreUtente('Nessun file .gpx ricevuto.');
            }
            ...
            try {
                ...
                // BASSO (giro agente sul fix B-2): stessa rete di sicurezza del ramo .fit sopra.
                if (!(e instanceof ErroreGpx) && !e.utente) console.error('parseGpx inatteso:', e);
                throw erroreUtente((e instanceof ErroreGpx || e.utente) ? e.message : 'File .gpx non leggibile.');
            }
        }
        ...
        if (dentro * 2 <= esaminati) {
            throw erroreUtente('Questa traccia si svolge fuori dalle quattro regioni coperte dal sito (Marche, Lazio, Abruzzo, Molise).');
        }

        // Traccia da un file di un dispositivo (.gpx/.fit): soglia 0, nessuna lisciatura -
        // il dislivello e' quello grezzo del file (decisione di Denis, 2026-09-06; il
        // perche' e i numeri misurati sono in lib/gpx.js sopra SOGLIA_DISLIVELLO_M).
        const stats = statisticheTraccia(letto.punti, SOGLIA_DISLIVELLO_IMPORT_M, haversineKm);
        if (stats.quotaMaxM === null) {
            throw erroreUtente(`Questo file .${routeSource.kind} non contiene le quote: non è possibile calcolare il dislivello.`);
        }

        // Punto 116: la geometria per disegnare la traccia sulla mappa (routePath sull'Hike).
        // Stesso trattamento che routes/routing.js fa per un SavedRoute, ora in una funzione
        // sola (routePathDaPunti, sopra): semplifica (~18 m), tieni solo [lng,lat], campiona a
        // passo fisso se restano troppi punti. La quota e' gia' stata usata sopra.
        const routePath = routePathDaPunti(letto.punti);
...
async function risolviPercorso(routeSource, userId) {
    ...
    try {
        ...
        // B-2 (follow-up revisione sicurezza, 31a): fail-closed come M-3 - solo un errore
        // marcato .utente (erroreUtente, sopra) e' sicuro da rimandare cosi' com'e' al
        // client; qualunque altra eccezione imprevista resta nel log del server.
        if (!(e && e.utente)) console.error('Errore risolviPercorso:', e);
        return { ok: false, status: 400, corpo: { error: (e && e.utente) ? e.message : 'Non è stato possibile calcolare questo percorso.' } };
    }
...
module.exports = { calcolaDaPercorso, risolviPercorso, misureDaSessione, routePathDaPunti, erroreUtente, MAX_BYTE_GPX_HIKE, CAMPIONE_REGIONE_HIKE, SEMPLIFICA_PERCORSO_M, MAX_PUNTI_PERCORSO };
