...
const { quoteDelPercorso, passoPerLinea } = require('./elevation');   // punto 33, punto 38
...
async function progettaPercorso(punti, opzioni = {}) {
    ...
    return {
        ...
        // Dal punto 38 si dice anche PERCHE' non c'e' (motivoDislivello): "fonte" oppure
        // "troppo lungo" sono due cose diverse, e la seconda non passa aspettando.
        dislivelloDisponibile: typeof quote.salitaM === 'number',
        ...quote
    };
}
...
async function dislivelloDelleTappe(tappe, opzioni = {}) {
    if (opzioni.saltaQuote) return { motivoDislivello: 'fonte' };   // prove: caso "fonte guasta"
    if (!tappe.length) return { motivoDislivello: 'fonte' };
    ...
    if (!esito) {
        // PUNTO 38 - PERCHE' NON BASTA DIRE "non c'e'".
        // Le quote si possono stimare fino a ~25 km (500 punti al passo massimo di 50 m, vedi
        // passoPerLinea). Oltre, quoteDelPercorso torna null ESATTAMENTE COME quando la fonte
        // non risponde - e il pannello scriveva "la fonte delle quote non ha risposto", che in
        // quel caso e' falso: la fonte sta benissimo, e' il percorso a essere troppo lungo.
        // Il difetto c'era gia', ma chiudere l'anello raddoppia la lunghezza e lo rende molto
        // piu' facile da incontrare: un giro da 13 km diventa 26 e ci finisce dentro.
        // Raccontare la causa sbagliata e' una bugia come le altre: chi legge "riprova fra
        // poco" riprova all'infinito su un percorso che non funzionera' mai.
        return { motivoDislivello: passoPerLinea(linea) === null ? 'troppo lungo' : 'fonte' };
    }
