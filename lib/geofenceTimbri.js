...
const { dataRomaISO } = require('./accountDeletion'); // "YYYY-MM-DD" in Europe/Rome, non UTC

const SOGLIA_TIMBRO_M = 150;

// Soglia di prossimita' di un SINGOLO punto. Quasi sempre i 150 m standard, ma una voce del
// catalogo (public/js/badge-points.js) puo' dichiararne una propria col campo `sogliaM`:
// oggi solo Bivacco Ju Busciu, a 10 m ("e' una perla nascosta", richiesta di Denis
// 2026-08-31). `sogliaM` assente o non finito => SOGLIA_TIMBRO_M.
// NB: la finestra in gradi di finestraGradi() resta dimensionata su SOGLIA_TIMBRO_M, che e'
// la soglia PIU' AMPIA e fa da pre-filtro per qualunque punto (anche quelli piu' stretti).
// Un punto con sogliaM > 150 romperebbe quel pre-filtro: non introdurne senza allargare li'.
const sogliaPunto = (p) => (p && Number.isFinite(p.sogliaM) ? p.sogliaM : SOGLIA_TIMBRO_M);
...
async function puntiTimbrabili() {
    ...
    // Il raggruppamento lo fa Atlas, non Node (verifica generale, 57a sessione): prima era
    // Hike.find({}) - OGNI escursione del database caricata in RAM a ogni chiamata (import
    // di un .gpx, timbro da mappa, salite di una vetta) per tenerne solo le vette fuori
    // catalogo. Stessa forma del difetto che ha gia' fatto cadere il sito con l'indice
    // sentieri. Ora a Render arriva un documento per stampId distinto, qualunque sia il
    // numero di escursioni. $first senza ordinamento = la prima escursione in ordine
    // naturale, come faceva il ciclo di prima.
    const vette = await Hike.aggregate([
        { $match: { 'peaks.stampId': { $exists: true } } },
        { $unwind: '$peaks' },
        { $match: {
            'peaks.stampId': { $nin: [null, '', ...perCodice.keys()] },
            'peaks.lat': { $type: 'number' },
            'peaks.lng': { $type: 'number' }
        } },
        { $group: { _id: '$peaks.stampId', name: { $first: '$peaks.name' }, lat: { $first: '$peaks.lat' }, lng: { $first: '$peaks.lng' } } }
    ]);
    for (const v of vette) {
        if (!isFiniteNum(v.lat) || !isFiniteNum(v.lng)) continue; // $type 'number' lascia passare NaN
        perCodice.set(v._id, { stampId: v._id, nome: v.name || 'Punto senza nome', emoji: '⛰️', lat: v.lat, lng: v.lng });
    }
...
// Vero se almeno un punto della traccia sta entro la soglia del punto (sogliaPunto: 150 m,
// oppure quella propria della voce di catalogo - Ju Busciu 10 m). Non calcola il minimo: si
// ferma al primo punto dentro soglia. Da preferire quando serve solo il si'/no (contare
// quante sessioni distinte toccano una vetta; decidere se un timbro da mappa e' legittimo).
// `punto` e' l'oggetto del catalogo {lat, lng, sogliaM?} - non bastano lat/lng sciolte,
// serve sogliaM. Chiamanti: routes/stamps.js, routes/tracking.js (/peak-ascents).
function tracciaToccaPunto(punti, punto) {
    const soglia = sogliaPunto(punto);
    const { gradiLat, gradiLng } = finestraGradi(punto.lat);
    return (punti || []).some(p =>
        Math.abs(p[1] - punto.lat) <= gradiLat &&
        Math.abs(p[0] - punto.lng) <= gradiLng &&
        haversineKm(p[1], p[0], punto.lat, punto.lng) * 1000 <= soglia
    );
}
...
async function assegnaTimbriDallaTraccia(userId, punti, dataUscita) {
    try {
        ...
        for (const c of candidati) {
            ...
            if (minimo <= sogliaPunto(c)) raggiunti.set(c.stampId, { punto: c, distanzaM: Math.round(minimo) });
        }
        ...
        // stesso fuso Europe/Rome (mai UTC, vicino a mezzanotte sposterebbe il giorno).
        // ATTENZIONE: dataRomaISO() converte in silenzio un non-Date in "oggi" (nessun
        // errore) - dataUscita deve restare una Date vera, mai la stringa "YYYY-MM-DD"
        // di hike.date, o il timbro si data oggi invece che il giorno dell'escursione.
        const giorno = dataRomaISO(dataUscita);
