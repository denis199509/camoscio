...
async function riassegnaAdminSquadre(userId) {
    ...
    for (const squad of squads) {
        ...
        // Il creatore e' un admin "vivo"? Se e' lui che se ne va, no. Se e' un altro, si
        // controlla che non sia gia' eliminato a sua volta (catena rara ma possibile).
        ...
        if (creatoreVivo || squad.admins.length > 0) {
            await squad.save();
            continue;
        }

        // Nessun admin vivo: promuovi il primo membro vivo. L'ordine dell'array members
        // e' l'ordine d'iscrizione ($addToSet in approvazione - vedi routes/squads.js).
        let promosso = null;
