...
// "YYYY-MM-DD" nel fuso Europe/Rome, di una Date qualsiasi (default: adesso). Hike.date e'
// salvato cosi' dal frontend (data locale, non UTC): confrontarlo o costruirlo con UTC
// sballerebbe di qualche ora vicino a mezzanotte (fino a 2h, ora legale). 'sv-SE' produce
// proprio il formato ISO YYYY-MM-DD.
// Esportata anche a routes/hikes.js/routes/stamps.js (blocco iscrizioni, timbro sbloccato
// oggi) e lib/geofenceTimbri.js (data dell'uscita, non "oggi" - da qui l'argomento):
// helper generico, non specifico dell'eliminazione account - vive qui perche' era l'unico
// punto che lo usava quando e' nato, si sposta in un lib/ suo se cresce ancora.
function dataRomaISO(date) {
...
async function scrubAccount(user) {
    ...
    const esito = await User.updateOne(
        { _id: user._id, deletedAt: { $exists: false } },
        {
            ...
            $unset: {
                ...
                deadManLastFired: 1, // BASSO-3 (35a): puo' contenere nomi di terzi (contatti non raggiunti)
                canModerateReports: 1, receivesReportAlerts: 1,
                // Blocco 2 del piano 2FA: un account scrubato non deve tenersi addosso il
                // segreto TOTP ne' le impronte dei codici di recupero (stesso genere di
                // regressione dell'ALTO della 31a - l'export che aveva smesso in silenzio di
                // contenere la foto). $unset di un campo assente e' un no-op: innocuo finche'
                // nessuno accende il 2FA. La collezione AccountRecovery (recupero ritardato,
                // opzione C) si aggiunge ai deleteMany qui sotto nel BLOCCO 5, quando quel
                // modello esistera'.
    ...
    // Solo le notifiche DI questo utente. Quelle SU DI LUI ("Mario Rossi ha chiesto di...")
    // gia' finite nelle liste altrui non si raggiungono da qui: il loro `text` e' una stringa
    // libera senza un id dell'attore. Le porta via il TTL di 90 giorni (models/Notification.js) -
    // il nome resta al massimo per quel tempo, non a tempo indefinito.
    await Notification.deleteMany({ userId: user._id });
    await EmailVerification.deleteMany({ userId: user._id });
    await PasswordReset.deleteMany({ userId: user._id });
    // Blocco 5 del piano 2FA: un recupero ritardato in sospeso (o gia' annullato/completato -
    // annullatoIl esiste apposta per tenere la storia) resterebbe addosso a un account
    // scrubato a tempo indeterminato. Stesso genere di regressione dell'ALTO della 31a.
    await AccountRecovery.deleteMany({ userId: user._id });
    ...
    // ALTO (verifica generale, blocco 2, 44a sessione) e corretto qui (46a): STESSA
    // geometria di sopra, copiata su Hike.routePath (punto 116) quando questo utente ha
    // creato un'escursione da un .gpx/.fit/percorso salvato - restava intatta dopo lo
    // scrub perche' qui non veniva toccata. routeSource (l'etichetta kind/nome) e i tre
    // numeri (distanceKm/elevationGain) restano: sono gia' solo un'etichetta e un totale,
    // non la linea grezza.
    await Hike.updateMany(
        { creatorId: user._id },
        { $unset: { routePath: 1 } }
    );
    ...
    // Verifica generale, blocco 2 (44a), chiuso nella 56a: altri due dati personali restavano
    // su documenti di TERZI, fuori da tutto quello che c'e' sopra.
    // (1) Il passaggio offerto sulle escursioni altrui: departureCity e' il comune da cui
    // parte, cioe' vicino a casa sua. Si toglie l'INTERA offerta, non solo il comune: sulle
    // escursioni future restava un'auto con "Sali a Bordo" attivo guidata da "Account
    // eliminato", che non arrivera' mai (regola: non promettere a schermo). E lo si toglie
    // dalle auto altrui in cui era passeggero - un posto occupato da nessuno.
    await Hike.updateMany({ 'carpool.drivers.userId': user._id }, { $pull: { 'carpool.drivers': { userId: user._id } } });
    await Hike.updateMany(
        { 'carpool.drivers.passengers': user._id },
        { $pull: { 'carpool.drivers.$[].passengers': user._id } }
    );
    // (2) "da <nome>" sui percorsi che altri hanno copiato dalle sue tracce: origineUsername
    // e' una COPIA del nome fatta al salvataggio (A-3.4, routes/routing.js) - la
    // pseudonimizzazione di User non la raggiunge. Stesso testo che il salvataggio usa gia'
    // per un autore eliminato.
    await SavedRoute.updateMany({ origineUserId: user._id }, { $set: { origineUsername: NOME_ELIMINATO } });
