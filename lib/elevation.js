...
async function quoteDelPercorso(coordinate) {
    try {
        ...
        // Le quote di QUESTA richiesta si tengono in una mappa sua, non si rileggono dalla
        // memoria condivisa alla fine (verifica generale, 57a sessione): fra la lettura e la
        // fine ci sono gli await verso la fonte, e intanto un'altra richiesta puo' riempire
        // la memoria e sfrattare proprio le quote gia' note qui - un buco, e il dislivello
        // tornava null in silenzio anche se le quote erano state pagate. La memoria condivisa
        // resta solo il risparmio FRA richieste diverse. La mappa vive quanto la richiesta ed
        // e' grande quanto `punti`, gia' limitati da passoPerLinea.
        const quoteQui = new Map();
        const daChiedere = [];
        for (const p of punti) {
            const chiave = chiaveDi(p);
            if (quoteQui.has(chiave)) continue;
            if (quoteRicordate.has(chiave)) quoteQui.set(chiave, quoteRicordate.get(chiave));
            else { quoteQui.set(chiave, undefined); daChiedere.push(p); } // si chiedono SOLO quelle ignote
        }
        for (let i = 0; i < daChiedere.length; i += MAX_PUNTI_PER_CHIAMATA) {
            ...
            gruppo.forEach((p, k) => { quoteQui.set(chiaveDi(p), parziale[k]); ricorda(p, parziale[k]); });
        }

        const quote = punti.map(p => quoteQui.get(chiaveDi(p)));
