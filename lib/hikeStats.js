...
async function applyHikeCompletionStats(user, hike, tempi) {
    ...
    // Il findOne sopra non basta da solo: fra lui e la create puo' arrivare un'altra richiesta
    // per la stessa coppia (doppio tocco, oppure l'auto-completamento che corre col
    // completamento di gruppo). L'indice unico userId+hikeId la ferma con E11000: e' lo stesso
    // caso "gia' completata" di sopra, non un errore - senza questa guardia la rotta singola
    // rispondeva 400 invece di 409, e complete-group si interrompeva a meta' ciclo con
    // l'escursione gia' chiusa, lasciando senza Completion i confermati rimasti.
    try {
...
async function ensureCompletionReminders(userId) {
    ...
    const oggi = new Date();
    const gg = (d) => `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
    const todayStr = gg(oggi);
    // BASSO-2 (revisione 35a): SOLO le escursioni scadute negli ultimi ~60 giorni. Prima il
    // limite era solo `< oggi`, ma le Notification hanno un TTL di 90 giorni
    // (models/Notification.js): passati i 90, il promemoria spariva e la guardia anti-doppione
    // (giaAvvisato, sotto) non lo trovava piu' -> lo ricreava, e cosi' ogni 90 giorni, per
    // sempre, per ogni escursione mai chiusa in gruppo. La finestra a 60 giorni (< 90 del TTL)
    // chiude il ciclo: quando il promemoria scade, l'escursione e' gia' fuori da questa query.
    // E' anche coerente col senso del promemoria (un sollecito "il giorno dopo", non un
    // rimprovero perenne su una gita di sei mesi fa).
    const GIORNI_FINESTRA_PROMEMORIA = 60;
    const limiteStr = gg(new Date(oggi.getTime() - GIORNI_FINESTRA_PROMEMORIA * 24 * 60 * 60 * 1000));
    ...
    const scadute = await Hike.find({
        ...
        date: { $gte: limiteStr, $lt: todayStr }
    });
