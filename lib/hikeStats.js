...
// Valori di default dello schema User (models/User.js) - unica fonte anche per lo script di
// manutenzione (scripts/ricalcola-passo-personale.js) e per recalculatePersonalPace sotto,
// cosi' i tre posti che finora ripetevano "350"/"500" restano uno solo.
...
// chiamante a decidere se e quando applicarli. Necessario perche' non tutti i chiamanti
// devono scrivere quando le osservazioni sono zero - lo script di manutenzione lascia
// invariati gli utenti demo del seed (passo assegnato a mano, nessun Completion a
// giustificarlo): scriverci sopra un --scrivi li resetterebbe silenziosamente al default.
async function recalculatePersonalPace(userId) {
    ...
    if (osservazioni.length === 0) {
        return { osservazioni, nuovoPaceUp: DEFAULT_PACE_UP, nuovoPaceDown: DEFAULT_PACE_DOWN };
    }
...
async function applyHikeCompletionStats(user, hike, tempi) {
    ...
    // vuoto e il passo non deve muoversi.
