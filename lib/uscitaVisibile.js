...
async function guardiaUscitaVisibile(sessionId, viewerId, campi) {
    // Senza chi guarda la guardia va chiusa, non aperta: il driver MongoDB toglie dal filtro
    // un campo undefined, quindi Follow.exists({ followerId: undefined, followingId }) diventa
    // "l'autore ha almeno un follower" (verificato serializzando il filtro, 57a sessione).
    // Oggi tutte le rotte chiamanti stanno dietro requireAuth: questa e' la difesa se una
    // rotta nuova se ne dimenticasse.
    if (!viewerId) {
        return { errore: { status: 401, body: { error: 'Devi accedere per vedere questa uscita.' } } };
    }
