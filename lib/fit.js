...
// Tetto PROPRIO del .fit, contato PRIMA di decodificare (verifica generale, 57a sessione,
// valore scelto da Denis). MAX_PUNTI da solo non bastava: la libreria costruisce un oggetto
// per OGNI messaggio del file (record, eventi, hrv...) e il conteggio arrivava solo dopo.
// Misurato: un .fit costruito apposta da 7 MB (record da 13 byte) portava il processo a
// +221 MB prima di essere rifiutato, su 512 MB di Render. 100.000 messaggi sono piu' di 27
// ore a un punto al secondo; il caso peggiore misurato a questo tetto e' ~+140 MB.
const MAX_MESSAGGI_FIT = 100000;

// Scorre il binario e conta i messaggi dati SENZA creare oggetti: legge solo le intestazioni
// e le dimensioni dichiarate dalle definizioni, come fa fit-file-parser (readRecord in
// node_modules/fit-file-parser/dist/binary.js) per sapere dove comincia il messaggio dopo.
// Su un file strutturalmente rotto smette di contare e lascia decidere alla libreria, che
// ha i suoi messaggi d'errore: questa e' solo la guardia sulla quantita'.
function controllaQuantitaFit(buffer) {
    const lungIntestazione = buffer[0];
    if ((lungIntestazione !== 12 && lungIntestazione !== 14) || buffer.length < lungIntestazione) return;
    const fine = lungIntestazione + buffer.readUInt32LE(4);
    if (fine > buffer.length) return;

    const dimensioneDati = new Array(16).fill(null); // per tipo di messaggio locale (0-15)
    let i = lungIntestazione;
    let messaggi = 0;
    while (i < fine) {
        const h = buffer[i];
        let locale;
        if (h & 0x80) {
            locale = (h & 0x60) >> 5;                  // orario compresso
        } else if (h & 0x40) {                          // definizione
            const nCampi = buffer[i + 5];
            let prossimo = i + 6 + nCampi * 3;
            if (nCampi === undefined || prossimo > fine) return;
            let dimensione = 0;
            for (let k = 0; k < nCampi; k++) dimensione += buffer[i + 6 + k * 3 + 1];
            if (h & 0x20) {                             // campi developer
                const nDev = buffer[prossimo];
                if (nDev === undefined || prossimo + 1 + nDev * 3 > fine) return;
                for (let k = 0; k < nDev; k++) dimensione += buffer[prossimo + 1 + k * 3 + 1];
                prossimo += 1 + nDev * 3;
            }
            dimensioneDati[h & 0x0F] = dimensione;
            i = prossimo;
            continue;
        } else {
            locale = h & 0x0F;
        }
        const dimensione = dimensioneDati[locale] ?? dimensioneDati[0]; // stesso ripiego della libreria
        if (dimensione === null) return;
        if (++messaggi > MAX_MESSAGGI_FIT) {
            throw new ErroreFit(`Il file contiene piu' di ${MAX_MESSAGGI_FIT.toLocaleString('it-IT')} messaggi: oltre 27 ore registrate a un punto al secondo, troppe per un'escursione.`);
        }
        i += 1 + dimensione;
    }
}
...
async function parseFit(buffer) {
    ...
    controllaQuantitaFit(buffer); // PRIMA della libreria: vedi MAX_MESSAGGI_FIT
