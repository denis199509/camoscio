...
// Allarme vero del Dead Man's Switch (punto 37), mandata al CONTATTO scelto, non a chi ha
// attivato il timer. Deve dire una cosa sola con certezza (l'orario previsto e' passato senza
// check-in) ed essere onesta su cosa il sito NON fa: non chiama i soccorsi, non e' un servizio
// di emergenza - e' un promemoria automatico, l'unica azione resta di chi legge (vincolo hard 7,
// stesso principio gia' applicato al tasto SOS che apre tel:112 invece di promettere il satellite).
function emailAllarmeDeadMan({ nomeContatto, nomeEscursionista, oraAttesa, posizioneTesto }) {
    ...
    const html = `
        <p>${saluto}</p>
        <p>sei il contatto di emergenza indicato da <strong>${nomeEscursionista}</strong> su Camoscio, un'app di escursionismo.</p>
        <p><strong>${nomeEscursionista}</strong> aveva impostato un promemoria di rientro per le <strong>${oraAttesa}</strong> e non ha fatto il check-in entro quell'ora.</p>
        <p style="padding:12px 16px;background:#fdf1ec;border-left:4px solid #A83B2E;border-radius:4px;">Ultima posizione nota:<br><strong>${posizioneTesto}</strong></p>
    ...
    return { oggetto: `Camoscio - ${nomeEscursionista} non ha fatto il check-in`, testo, html };
}
