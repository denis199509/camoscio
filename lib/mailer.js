...
// Le sole credenziali Mailjet: basta per spedire un messaggio che non contiene NESSUN link
// (es. l'allarme del Dead Man's Switch, vedi ALTO qui sotto). Separata da configurato() -
// che in piu' vuole anche un APP_BASE_URL utilizzabile - perche' quel requisito serve SOLO
// alle email con un link dentro, non a questa.
function credenzialiPresenti() {
    return !!(
...
function configurato() {
    return credenzialiPresenti() && indirizzoBaseUtilizzabile();
}
...
// ALTO (verifica generale, blocco 2, 44a sessione) e corretto qui (46a): il corpo grezzo
// della risposta Mailjet puo' contenere l'ErrorMessage del servizio, che su un rifiuto
// legato al destinatario puo' ripetere l'indirizzo stesso - loggato per intero finiva in
// chiaro nei log di Render (retention fuori dal nostro controllo, stessa preoccupazione
// gia' scritta altrove nel progetto per il duplicato di registrazione). Si redige
// l'indirizzo del destinatario prima di loggare, invece di fidarsi che Mailjet non lo
// ripeta mai: non serve conoscere lo schema esatto dei loro errori per essere al sicuro.
function corpoErroreSenzaDestinatario(corpo, a) {
    let testo = JSON.stringify(corpo).slice(0, 500);
    if (a) {
        const escapedA = String(a).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        testo = testo.replace(new RegExp(escapedA, 'gi'), '[email-destinatario-redatta]');
    }
    return testo;
}
 ...
 *
 * richiedeLinkFunzionante (ALTO, verifica generale blocco 2, 44a sessione, corretto qui
 * 46a): di default true, perche' la maggior parte delle email (recupero password, verifica
 * indirizzo, recupero ritardato) contiene un link costruito con APP_BASE_URL - se quella
 * variabile manca o e' sbagliata, mandarle sarebbe peggio che non mandarle (un link rotto
 * sembra un'email riuscita). L'allarme del Dead Man's Switch (routes/safety.js) passa
 * false: non contiene NESSUN link, quindi non ha motivo di dipendere da una variabile che
 * serve solo a costruirne uno - prima ne dipendeva comunque (stesso cancello di tutte le
 * altre email), e un APP_BASE_URL rotto in produzione faceva tacere silenziosamente
 * l'unico avviso di sicurezza del sito, oltre a mandare posizione GPS ed email dei
 * contatti d'emergenza nel ramo "stampa sul terminale" qui sotto invece che via Mailjet.
 */
async function inviaEmail({ a, oggetto, testo, html, richiedeLinkFunzionante = true }) {
    ...
    if (!(richiedeLinkFunzionante ? configurato() : credenzialiPresenti())) {
        console.log('\n=============================================================');
    ...
    try {
        ...
        if (!res.ok) {
            ...
            // indovinare.
            console.error(`Invio email fallito (${res.status}):`, corpoErroreSenzaDestinatario(corpo, a));
        ...
        if (!esito || esito.Status !== 'success') {
            console.error('Invio email rifiutato dal servizio:', corpoErroreSenzaDestinatario(corpo, a));
            ultimoInvioRiuscito = false;
            return false;
        }
...
function emailRecuperoPassword({ nome, link, durataMinuti }) {
    ...
    const html = `
        ...
        <p><a href="${esc(link)}" style="display:inline-block;padding:12px 20px;background:#3E6B48;color:#fff;text-decoration:none;border-radius:8px;">Scegli una nuova password</a></p>
        <p style="font-size:13px;color:#555;">Se il pulsante non funziona, copia questo indirizzo nel browser:<br>${esc(link)}</p>
...
// Avviso di RECUPERO RITARDATO (opzione C, blocco 5 del piano 2FA). Mandata quando qualcuno
// avvia un recupero dell'accesso SENZA il secondo fattore. Diversa dalle altre per una
// ragione di progetto: qui l'email E' meta' dell'avviso, non una cortesia - avviare in
// silenzio un conto alla rovescia verso la presa di un account e' il vincolo hard 7 al
// rovescio, quindi in routes/auth.js il suo fallimento fa fallire la richiesta (503).
// L'altra meta', e quella che conta se la casella e' gia' compromessa, e' il BANNER in-app.
// dataMaturita arriva GIA' formattata IN ORA ITALIANA dalla rotta (errore gia' fatto due
// volte nel progetto, 19/08 e 21/08).
function emailRecuperoRitardato({ nome, dataMaturita, linkCompletamento }) {
    const saluto = nome ? `Ciao ${nome},` : 'Ciao,';
    const testo = [
        saluto,
        '',
        'qualcuno ha avviato un recupero dell\'accesso per il tuo account Camoscio, SENZA il',
        'codice del secondo fattore.',
        '',
        `Se sei stato tu, non devi fare niente: il ${dataMaturita} questo link comincera' a`,
        'funzionare e potrai scegliere una password nuova. Prima di allora non fa niente.',
        '',
        linkCompletamento,
        '',
        'SE NON SEI STATO TU, entra su Camoscio con la tua password e il tuo codice e annulla',
        'il recupero. Lo trovi in Impostazioni -> Sicurezza, e lo vedi anche in cima a ogni',
        `pagina del sito. Fino al ${dataMaturita} il tuo accesso non cambia.`,
        '',
        '--',
        'Camoscio - escursioni in montagna'
    ].join('\n');
    ...
    const html = `
        <p>${esc(saluto)}</p>
        <p>qualcuno ha avviato un recupero dell'accesso per il tuo account <strong>Camoscio</strong>, <strong>senza</strong> il codice del secondo fattore.</p>
        <p>Se sei stato tu, non devi fare niente: <strong>il ${esc(dataMaturita)}</strong> questo link comincer&agrave; a funzionare e potrai scegliere una password nuova. Prima di allora non fa niente.</p>
        <p style="font-size:13px;color:#555;">Copia questo indirizzo nel browser quando sar&agrave; il momento:<br>${esc(linkCompletamento)}</p>
        <p style="padding:12px 16px;background:#fdf1ec;border-left:4px solid #A83B2E;border-radius:4px;"><strong>Se NON sei stato tu</strong>, entra su Camoscio con la tua password e il tuo codice e annulla il recupero. Lo trovi in Impostazioni &rarr; Sicurezza, e lo vedi anche in cima a ogni pagina. Fino al ${esc(dataMaturita)} il tuo accesso non cambia.</p>
        <p style="font-size:12px;color:#888;">&mdash;<br>Camoscio &middot; escursioni in montagna</p>
    `;

    return { oggetto: 'Camoscio - richiesta di recupero accesso', testo, html };
}
...
function emailVerificaIndirizzo({ nome, link, durataOre }) {
    ...
    const html = `
        ...
        <p><a href="${esc(link)}" style="display:inline-block;padding:12px 20px;background:#3E6B48;color:#fff;text-decoration:none;border-radius:8px;">Conferma il mio indirizzo</a></p>
        <p style="font-size:13px;color:#555;">Se il pulsante non funziona, copia questo indirizzo nel browser:<br>${esc(link)}</p>
...
module.exports = {
    inviaEmail, emailRecuperoPassword, emailRecuperoRitardato, emailVerificaIndirizzo,
    emailAllarmeDeadMan, indirizzoBase, configurato, inviiFunzionanti
};
