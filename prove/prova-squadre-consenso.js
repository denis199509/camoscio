...
(async () => {
    ...
    try {
        ...
        // MEDIO-2 (follow-up revisione sicurezza): la validazione legge i BYTE veri, non piu'
        // solo la lunghezza della stringa - MEDIO-3 residuo ha poi ristretto il formato al
        // solo JPEG (il client comprime sempre, come le segnalazioni). Il fixture deve avere
        // una firma JPEG vera (FF D8 FF) seguita da riempimento qualunque - il server non
        // decodifica l'immagine per intero, solo la firma + la dimensione, stesso spirito di
        // Report.photo (routes/reports.js).
        const jpegMagic = Buffer.from([0xFF, 0xD8, 0xFF]);
        const fintoDataUrl = 'data:image/jpeg;base64,' + Buffer.concat([jpegMagic, Buffer.from('A'.repeat(200))]).toString('base64');
        ...
        // "data:image/jpeg" e sotto il tetto di lunghezza (prima passava lo stesso: si
        ...
        // MEDIO-3 residuo: un formato diverso da JPEG (qui PNG, con una firma vera) e' ora
        // rifiutato - prima di questo giro (senza compressore lato client) sarebbe passato.
        const pngMagic = Buffer.from([0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A]);
        const pngVero = 'data:image/png;base64,' + Buffer.concat([pngMagic, Buffer.from('A'.repeat(200))]).toString('base64');
