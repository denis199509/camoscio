// Punto 98/E (riaperto 21/08/2026) - PRIMA di decidere se includere highway=residential nel
// livello i, si misura quanto pesa: quante strade residential cadono vicine ai sentieri GIA'
// conosciuti, per regione e per soglia di distanza. SOLA LETTURA: nessuna scrittura sulla
// collezione Trail, nessuna modifica al filtro vero (scripts/fetch-trails-nearby.js). Il
// risultato serve solo a decidere con i numeri in mano fra le tre strade proposte a Denis
// (misurare / aggiungere con raggio stretto solo per questo tag / lasciare stare).
//
// Perche' residential non c'e' gia' nel filtro: mai incluso (nemmeno nei 7 filtri del
// 27/07/2026), per il rischio di tirare dentro reti stradali di paesi veri vicino ai punti
// di partenza dei sentieri - lo stesso motivo per cui fu scartato il filtro "tutte le
// strade". Il caso vero che ha riaperto il punto: Valle Granara, dove la strada che serve
// e' taggata residential (4 trovate entro 600 m, ma mai misurate sulle altre regioni).
//
// METODO: stessa griglia a celle di fetch-trails-nearby.js ("vicino a un sentiero noto" =
// dentro la cella del punto o una delle 8 vicine, cella grande quanto la soglia) - quindi il
// numero per soglia e' un limite superiore un po' largo, non una distanza esatta, la STESSA
// approssimazione gia' accettata per il filtro in produzione. Ogni way finisce nella soglia
// PIU' PICCOLA che lo cattura (le soglie sono crescenti): un conteggio "entro-200" non
// include quindi chi e' gia' contato in "entro-150".
//
// RIPARTIBILE come lo script gemello: progresso salvato riquadro per riquadro in
// data/misura-residential-progresso.json. Interruzione = nessun dato perso, si rilancia e
...
const FILE_PROGRESSO = path.join(__dirname, '..', 'data', 'misura-residential-progresso.json');
const FILE_REPORT = path.join(__dirname, '..', 'data', 'misura-residential-report.json');
...
function leggiProgresso() {
    if (DACCAPO || !fs.existsSync(FILE_PROGRESSO)) return { fatti: [], perRegioneSoglia: {}, viste: 0 };
    try { return JSON.parse(fs.readFileSync(FILE_PROGRESSO, 'utf8')); }
    catch { return { fatti: [], perRegioneSoglia: {}, viste: 0 }; }
}
...
function scriviReport(perRegioneSoglia, viste) {
    ...
    console.log(`Way "highway=residential" visti dentro le 4 regioni, vicino a sentieri gia' conosciuti: ${viste.toLocaleString('it-IT')}\n`);
...
way["highway"="residential"](${riquadro.sud.toFixed(4)},${riquadro.ovest.toFixed(4)},${riquadro.nord.toFixed(4)},${riquadro.est.toFixed(4)});
out geom;`;
...
(async () => {
    ...
    for (let n = 0; n < elenco.length; n++) {
        ...
        fatti.add(r.k);
        scriviProgresso({ fatti: [...fatti], perRegioneSoglia, viste });

        const trascorso = (Date.now() - inizio) / 1000;
        const rimasti = Math.round((trascorso / (n + 1)) * (elenco.length - n - 1) / 60);
        console.log(`  [${n + 1}/${elenco.length}] ${r.k}: ${vie.length} residential visti | totale ${viste.toLocaleString('it-IT')} | ~${rimasti} min rimasti`);
